...
export function AnimatedAIChat({ onSend, loading: externalLoading }: AnimatedAIChatProps) {
    ...
    const commandSuggestions: CommandSuggestion[] = [
        {
            ...
            prefix: "¿Cuáles son los 10 productos más vendidos?"
        },
        {
            icon: <Sparkles className="w-4 h-4" />,
            label: "Clientes por país",
            description: "Cantidad de clientes agrupados por país",
            prefix: "Cantidad de clientes por país ordenado de mayor a menor"
        },
        {
            icon: <Sparkles className="w-4 h-4" />,
            label: "Ventas por categoría",
            description: "Revenue total agrupado por categoría",
            prefix: "Ventas totales agrupadas por categoría de producto"
        },
        {
            icon: <Sparkles className="w-4 h-4" />,
            label: "Órdenes pendientes",
            description: "Listado de órdenes con estado pendiente",
            prefix: "Mostrar todas las órdenes con estado pendiente"
        },
    ];
    ...
    return (
        <div className="min-h-screen flex flex-col w-full items-center justify-center bg-transparent text-foreground p-6 relative overflow-hidden">
            <div className="absolute inset-0 w-full h-full overflow-hidden">
                <div className="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500/15 rounded-full mix-blend-normal filter blur-[128px] animate-pulse" />
                <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-cyan-500/15 rounded-full mix-blend-normal filter blur-[128px] animate-pulse delay-700" />
                    ...
                    >
                        ...
                        <div className="p-4">
                            <Textarea
                                ...
                                placeholder="Ej: ¿Cuáles son los productos con más stock?"
                        ...
                        <div className="p-4 border-t border-border/40 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    ...
                                    <motion.span
                                        className="absolute inset-0 bg-primary/[0.06] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                        {commandSuggestions.map((suggestion, index) => (
                            <motion.button
                                key={suggestion.prefix}
                                onClick={() => selectCommandSuggestion(index)}
                                className="flex items-center gap-2 px-3 py-2 bg-muted/40 hover:bg-primary/10 rounded-lg text-sm text-muted-foreground hover:text-primary transition-all relative group border border-border/60"
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: index * 0.08 }}
                                title={suggestion.prefix}
                            >
                                {suggestion.icon}
            ...
            <AnimatePresence>
                {isLoading && (
                    <motion.div
                        className="fixed bottom-8 mx-auto transform -translate-x-1/2 backdrop-blur-2xl bg-card/90 rounded-full px-4 py-2 shadow-lg border border-border"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 20 }}
                    >
            ...
            {inputFocused && (
                <motion.div
                    className="fixed w-[50rem] h-[50rem] rounded-full pointer-events-none z-0 opacity-[0.04] bg-cyan-500 blur-[96px]"
        ...
        <div className="flex items-center ml-1">
            {[1, 2, 3].map((dot) => (
                <motion.div
                    ...
                    style={{ boxShadow: "0 0 4px hsl(var(--primary) / 0.4)" }}
                />
            ))}
        </div>
    );
...
export function ActionButton({ icon, label }: ActionButtonProps) {
    ...
    return (
        <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-card hover:bg-muted/60 rounded-full border border-border text-muted-foreground hover:text-foreground transition-all relative overflow-hidden group"
        >
