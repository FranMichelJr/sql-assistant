    ...
    <div className={`w-full max-w-7xl mx-auto ${className}`}>
      ...
      <div className="bg-background border border-border/50 rounded-2xl overflow-hidden">
        ...
        <div className="grid grid-cols-7 gap-4 px-6 py-3 text-xs font-medium text-muted-foreground/70 uppercase tracking-wide bg-muted/15 border-b border-border/20">
          <div className="flex items-center gap-3">
            <div className="relative">
              <input
                ...
                className="w-4 h-4 rounded border-muted-foreground/40 focus:ring-muted-foreground/20 focus:ring-2 accent-muted-foreground bg-background"
              />
            </div>
            <span>Lead</span>
          </div>
          ...
          <div>Interest</div>
          <div>Probability</div>
          <div className="flex items-center gap-2 cursor-pointer select-none" onClick={handleSort}>
            Last Action
            <ChevronDown className={`w-4 h-4 transition-transform duration-150 ${sortOrder === "asc" ? "rotate-180" : ""}`} />
          </div>
        </div>
          ...
          {leads.map((lead, index) => (
            <motion.div key={lead.id} variants={rowVariants}>
              <div
                ...
                onMouseEnter={() => { setHoveredRow(lead.id); setHoveredAction(lead.id); }}
                onMouseLeave={() => { setHoveredRow(null); setHoveredAction(null); }}
              >
                {/* Lead Info */}
                <div className="flex items-center gap-3">
                  <input
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-medium text-foreground/90 truncate text-sm">{lead.name}</div>
